...
# 学習データの読み込み
def load_data():
    history_path = sorted(Path('./data').glob('*.history3'))[-1]
    with history_path.open(mode='rb') as f:
        return pickle.load(f)
# 学習データの読み込み
def load_all_data():
    history_path = sorted(Path('./data').glob('*.history3'))
    ret = []
    ...
    for i in range(0,RN_EPOCHS):
        ...
        for j in indexs:
            ...
            if len(x) == RN_BATCH_SIZE:
                ...
                loss_values = (1.0 * torch.sum((outputs - yv2) ** 2))
                loss = loss_values 
